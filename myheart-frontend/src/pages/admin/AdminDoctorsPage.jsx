...
function AdminDoctorsPage() {
  ...
  const latestDoctor = doctors.length > 0 ? doctors[0] : null
  ...
  const hero = (
    <section style={S.hero}>
      ...
      <div style={S.heroTop}>
        <div>
          <div style={S.heroEyebrow}>STAFF ADMINISTRATION</div>
          <h1 style={S.heroTitle}>
            Doctors <span style={S.heroName}>Management</span>
          </h1>
          <p style={S.heroSubtitle}>
            Add medical staff, organize departments, and open each doctor’s
            full activity record to review patients, appointments, records,
            and lab requests.
          </p>
        </div>

        <div style={S.heroStatBox}>
          <div style={S.heroMiniValue}>{doctors.length}</div>
          <div style={S.heroMiniLabel}>TOTAL DOCTORS</div>
        </div>
      </div>
    </section>
  )
  ...
  return (
    <AdminLayout title="Doctors Management" active="doctors" hero={hero}>
      <section style={S.summaryGrid}>
        <SummaryCard
          icon="🩺"
          label="TOTAL DOCTORS"
          value={doctors.length}
          sub="registered medical staff"
        />
        <SummaryCard
          icon="🏥"
          label="DEPARTMENTS"
          value={departmentCount}
          sub="covered specialties"
        />
        <SummaryCard
          icon="🆔"
          label="LATEST DOCTOR ID"
          value={latestDoctor ? `#${latestDoctor.id}` : "-"}
          sub="most recently listed doctor"
        />
        <SummaryCard
          icon="🔎"
      ...
      <section style={S.grid}>
        <div style={S.card}>
          <div style={S.cardTitleSmall}>CREATE DOCTOR</div>
        ...
        <div style={S.card}>
          <div style={S.tableHeader}>
            <div style={S.cardTitleSmall}>DOCTORS DIRECTORY</div>
            <input
              value={search}
          ...
          {loading ? (
            <div style={S.emptyState}>Loading doctors...</div>
          ) : filteredDoctors.length === 0 ? (
            <div style={S.emptyState}>No doctors found.</div>
          ) : (
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {filteredDoctors.map((doctor) => (
                    <tr key={doctor.id}>
                      <td style={S.td}>#{doctor.id}</td>
                      <td style={S.td}>
                        {doctor.first_name} {doctor.last_name}
                      </td>
                      <td style={S.td}>{doctor.department}</td>
                      <td style={S.td}>{doctor.phone || "-"}</td>
